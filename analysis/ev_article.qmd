...
method_comp = pd.read_excel(fractions_folder / "ccht_uf_vs_bio.xlsx", index_col=0)
```
...
```{python}
de_ccht_phases = pd.read_feather(Path(data_folder) / "filtered" / "de" / "voom_phases.feather")
de_ccht_group = pd.read_feather(Path(data_folder) / "filtered" / "de" / "voom_methods.feather")
```
...
volcano_chart = alt.Chart().mark_point()
```
