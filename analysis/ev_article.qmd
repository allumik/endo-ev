...
fractions_file_loc_comb = fractions_folder / "comb_fracs_raw.tsv"
```
...
frac_comb_raw = pd.read_table(fractions_file_loc_comb, sep="\t", index_col=0)
...
# add the color groupings 
x_cycle_colors = barplot.mark_rect(clip=False, width=12).encode(
    y=alt.value(0), # for some reason this works
    y2=alt.value(-4),
    yOffset=alt.value(-12),
    color=alt.Color("cyclephase:N").scale(scheme="set1")
  )
x_method_colors = barplot.mark_rect(clip=False, width=12).encode(
    y=alt.value(0), # for some reason this works
    y2=alt.value(-16),
    yOffset=alt.value(-24),
...
chart_den_x = (shoulder + arm1 + arm2).properties(height=50, width=160)
```
...
chart_den_y = (shoulder + arm1 + arm2).properties(width=50, height=500)
```
...
# Layer heatmap and colors
final_chart = (x_group_colors + y_group_colors + base_heatmap)
...
scatter = alt.Chart(pca_df).mark_point(
  ...
  color=alt.Color("cyclephase:N", title=legend_title) if color_field else "black",
  shape=alt.Shape("dataset:N") if style_field else "circle"
)
...
```{python}
(
  dendro_corrplot | biplot & dendro_barplot
