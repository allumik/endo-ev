...
cyclephase_remap = { "pro": "PE", "pre": "ES", "rec": "MS", "post": "LS" }
...
```{python}
phase_colors = {
  "Proliferative": "#006d2c", "PE": "#006d2c", "EP": "#6ed5ff", "LP": "#006d2c",
  "Pre-receptive": "#00BA38", "ES": "#00BA38", "Early-receptive": "#00BFC4",
  "Receptive": "#619CFF", "MS": "#619CFF",
...
global_color_ct = alt.Scale(domain=general_cells.index.unique().tolist(), scheme="tableau20")
# color scale for the groups
global_color_group = alt.Scale(domain=["Tissue", "UF-EV"], scheme="dark2")
...
barplot_ev_sum = alt.Chart(ccht_uf_pheno).mark_bar().encode(
  x=alt.X("cyclephase:N", sort=phases).title(""),
...
df_melted['sample_group'] = df_melted['samplename'].apply(lambda x: 'Tissue' if 'biopsy' in x else 'UF-EV')
...
featurecounts_chart = alt.Chart(df_melted).mark_boxplot(extent=1.5).encode(
  ...
  color=alt.Color('sample_group:N', title='Sample Group', scale=global_color_group).legend(orient="top", titleOrient="left"),
...
sc_ratios_prep = (
  ...
  .assign(
    cyclephase_mapped=lambda df: pd.Categorical(
      ...
      categories=["PE", "ES", "MS", "LS"], # Ensure correct order
      ordered=True
    )
  )
...
swapper = [("LS", "MS"), ("MS", "ES"), ("ES", "PE")]
comp_order = ["PE_vs_ES", "ES_vs_MS", "MS_vs_LS"]
cycle_order = ["PE", "ES", "MS", "LS"]
...
text_overlay_stars = base_text.mark_text(fontWeight="bold", fontSize=20, align="center", dy=-10).encode(
...
yaxis_color_strip = alt.Chart(stat_comb).mark_rect().encode(
  ...
  color=alt.Color("lineage:N", scale=global_color_lineage)
)
...
base_heatmap = alt.Chart(corr_mat).mark_rect().encode(
  ...
  color=alt.Color('correlation:Q').scale(scheme='magma', domain=[0, 1]) # Quantitative for correlation values, using magma colormap with 0-1 domain
)
...
x_group_colors = base_heatmap.mark_rect().encode(
  ...
  color=alt.Color("cyclephase:N", scale=global_color_phase),
