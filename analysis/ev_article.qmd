...
boxplots.interactive()  # Enable zoom/pan
```
...
morans_chart = chart_layers.facet(
  data=moran_pivot,
...
# To display in a Jupyter environment:
morans_chart.interactive() # Enable zooming and panning and tooltips and selection and other fun interactivity
```
...
```{python}
# Try to make a layout plot with the spacers
# placeholder image 
raw_st_example = spacer_with_text(height=300, width=200)
projection_example = spacer_with_text(height=300, width=200)
(
  (raw_st_example & projection_example) |
  (boxplots & pearson_corr & morans_chart)
)
```
