...
for phase in unique_cycle_phases:
  ...
  dendro_barplot_phase = create_dendro_barplot(
    ...
    plot_title=f"Phase: {phase}", # Shorter title for combined plot
    plot_width=max(150, len(frac_subset.index) * 20), # Adjust width dynamically
    plot_height=200 # Consistent height for side-by-side
  )
...
if phase_plots:
  combined_plot = alt.hconcat(*phase_plots).resolve_scale(
    color='shared' # Resolve color scales if they differ
  )
  combined_plot.show()
else:
