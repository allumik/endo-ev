...
base_chart = alt.Chart(df_melted).mark_bar().encode(
  y=alt.Y('samplename:N', title='Sample', sort=sample_order),
  x=alt.X('percentage:Q', title='Percentage (%)', scale=alt.Scale(domain=(.8, 1)), axis=alt.Axis(format='%')),
  ...
  height=alt.Step(12), # Adjust bar width by setting step
  width=250,
  title="Biotype Proportions per Sample (Unfiltered, Top 10 Biotypes)"
)

# Add a rule (line) at y=90%
rule = alt.Chart(pd.DataFrame({'percentage': [0.9]})).mark_rule(strokeDash=[5, 5], color='red').encode(
  x=alt.X('percentage:Q')
)

biotype_prop = (base_chart + rule)
biotype_prop.interactive() # Enable zooming and panning and tooltips and selection and other fun interactivity
```
...
```{python}
supp_1 = (
  biotype_prop | (umap_plot_unfilt & umap_plot) 
  ).resolve_scale(
