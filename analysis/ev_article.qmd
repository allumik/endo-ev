...
*edit 25 august* not necessary anymore, converted the samplenames from the beginning now

```{python}
resampler = pd.read_csv(
  Path(raw_data_folder) / "resampler.csv", index_col=0, names=["hut", "s"]
  ).s.to_dict()
```
...
phase_colors = {
  ...
  "Pre-receptive": "#00BA38", "ES": "#00BA38", 
  "Early-receptive": "#00BFC4", "Early-secretory": "#00BFC4",
  "Receptive": "#619CFF", "MS": "#619CFF", "Secretory": "#619CFF", 
  "Mid-secretory": "#619CFF",
...
stat_comb_prim = (
  pd.merge(
    ...
    padj_format=lambda x: np.select(
        [x["p-adj"] < 0.01, x["p-adj"] < 0.05, x["anova p-val"] < .05],
        ["FDR<0.01", "FDR " + x["p-adj"].round(2).astype(str), ""],
        default="n.s."
    )
...
# fig2.save(fig_loc / "fig_2.png", ppi=300) # save it in png too bc of font issues
...
boxplot = alt.Chart(filtered_df).mark_boxplot(extent='min-max').encode(
  ...
  height=180  # Adjust this value to reduce subplot height
)
...
# rename all CTR1_processed_data to CTR1 for better readability
moran_df['slide_id'] = moran_df['slide_id'].str.replace(r"processed_data", "", regex=True)
...
legend1 = g.fig.legend(handles=cycle_handles, title='Cycle Phase', bbox_to_anchor=(0.80, 1), loc='upper left', borderaxespad=0., frameon=False, alignment='left', fontsize=8)
...
legend2 = g.fig.legend(handles=comp_handles, title='Generated dataset', bbox_to_anchor=(0.80, 0.92), loc='upper left', borderaxespad=0., frameon=False, alignment='left', fontsize=8)
...
# print(heatmap_data.max().max(), heatmap_data.min().min()) # Check the range of Moran's I values for better color scaling if needed
# print(heatmap_data.query("celltype in ['uSMCs', 'eStromal']").filter(regex="_scRNA-seq$").T.mean()) # Check the range of Moran's I values for a specific cell type if needed
