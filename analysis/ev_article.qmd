...
# print out the resampler dictionary as a dataframe
# pd.DataFrame.from_dict(resampler, orient='index', columns=['new_name']).to_csv(Path(data_folder) / "resampler.csv")
ccht_uf_pheno = ccht_uf_pheno.rename(index=resampler)
ccht_uf_pheno_unfilt = ccht_uf_pheno_unfilt.rename(index=resampler)
ccht_uf_unfilt_raw = (ccht_uf_pheno_unfilt
  .assign(qc_pass=lambda x: x.index.isin(ccht_uf_pheno.index))
  .pipe(lambda x: x.to_csv(Path(data_folder) / "main_pheno.csv") or x)
  .pipe(lambda x: ccht_uf_unfilt_raw.rename(columns=resampler)))
