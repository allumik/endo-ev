...
barplot_sc_sum = alt.Chart(
  sc_dat.obs[["cyclephase", "samplename", "dataset", "celltype"]].value_counts(["cyclephase", "dataset", "celltype"]).reset_index()
).mark_bar().encode(
...
```{python}
# Define the mapping for cycle phases
phase_mapping = {
  "EP": "pro", "LP": "pro", "ES": "pre", "MS": "rec", "LS": "post"
}
...
sc_ratios_prep = (
  ...
  .assign(cyclephase_mapped=lambda df: df["cyclephase"].map(phase_mapping)) # Map to new phases
...
fractions_long = (
  ...
  .assign(
    cyclephase=lambda df: pd.Categorical(
      df["cyclephase"],
      categories=["pro", "pre", "rec", "post"], # Ensure correct order
      ordered=True
    )
  )
)
...
biplot, pca = biplot_fractions(
  ...
  style_field="group",
  text_limit = 0.02, # controls on how much labels are shown
  dims = (400, 400)
  )
biplot.show()
```
...
base_chart = alt.Chart(stat_comb).encode(
  x=alt.X("comparison:N", sort=comp_order).axis(labels=True, labelAngle=-45, ticks=True, title=None),
...
# set up basechart for the Y axis (previously X) and X axis (previously Y)
base_chart = alt.Chart(stat_comb).encode(
  x=alt.X("comparison:N", sort=comp_order).axis(labels=True, labelAngle=-45, ticks=True, title=None, titleY=25),
  y=alt.Y("celltype:N", sort=stat_comb_celltypes).axis(labels=False, ticks=False, title=None)
)

# the base heatmap
heatmap = base_chart.mark_rect().encode(
  color=alt.Color("meandiff:Q").scale(scheme="blueorange", domain=[-.2,.2]))
...
unique_cycle_phases = ccht_uf_pheno['cyclephase'].unique()
...
for i, phase in enumerate(unique_cycle_phases):
  ...
  dendro_barplot_phase = create_dendro_barplot(
    ...
    plot_width=max(150, len(frac_subset.index) * 20),
    plot_height=200,
...
combined_plot = alt.hconcat(*phase_plots).resolve_scale(
  color='shared'
)
combined_plot.show()
```
...
avg_cyclephase_correlation_matrix = (
  ...
  .mean()
  .reset_index()
  .rename(columns={'cyclephase': 'biopsy_cyclephase', 'cyclephase_y': 'uf_cyclephase', 'correlation': 'avg_correlation'})
)
...
heatmap_avg_corr = alt.Chart(avg_cyclephase_correlation_matrix).mark_rect().encode(
    ...
    color=alt.Color('avg_correlation:Q', scale=alt.Scale(scheme='viridis'), legend=alt.Legend(title="Avg. Correlation"))
).properties(
    title='Average Correlation between Biopsy and UF Samples by Cycle Phase',
    width=200,
    height=200
)
...
text_avg_corr = heatmap_avg_corr.mark_text(baseline='middle').encode(
    ...
    color=alt.condition(
        alt.datum.avg_correlation > 0.5,  # Adjust threshold for better contrast
        alt.value('white'),
        alt.value('black')
    )
)
...
volcano_base = alt.Chart(
  ...
  x=alt.X("diff:Q").scale(domain=[-.12, .12]).axis(title="Mean Proportion Difference (UF - Biopsy)", tickCount=4),
...
```{python}
(combined_plot & (final_chart | volcano)).resolve_scale(color="independent")
```
...
ordnung_x = corr_long[["x_vars", "cyclephase"]].sort_values("cyclephase", key=lambda x: pd.Categorical(x, categories=cycle_order, ordered=True)).x_vars.unique()
ordnung_y = corr_long[["y_vars", "cyclephase_y"]].sort_values("cyclephase_y", key=lambda x: pd.Categorical(x, categories=cycle_order, ordered=True)).y_vars.unique()
# Create Altair plot
base_heatmap = alt.Chart(corr_long).mark_rect().encode(
    x=alt.X('x_vars:O', sort=ordnung_x, title=None).axis(offset=10),
    y=alt.Y('y_vars:O', sort=ordnung_y, title=None).axis(orient="right", offset=10),
    color=alt.Color('correlation:Q').scale(scheme='magma') # Quantitative for correlation values, using magma colormap
  )

# add the color groupings 
x_group_colors = base_heatmap.mark_rect().encode(
  x=alt.X('x_vars:O', sort=ordnung_x, title=None), # Nominal for categorical x-axis
  color=alt.Color("cyclephase:N"),
  xOffset=alt.value(10), # set it to +10 of the plot height so that it would overlap
)

y_group_colors = base_heatmap.mark_rect().encode(
  y=alt.Y('y_vars:O', sort=ordnung_y, title=None), # Nominal for categorical x-axis
  color=alt.Color("cyclephase_y:N").legend(None),
  yOffset=alt.value(10), # set it to +10 of the plot height so that it would overlap
)

# Layer heatmap and colors
dendro_corrplot = (x_group_colors + y_group_colors + base_heatmap).properties(width=150, height=600)
...
```{python}
sc.pl.embedding(gen_ccht_ev, basis='X_umap', color='celltype', title="EV pSC UMAP", legend_loc="on data", legend_fontsize=7)
sc.pl.embedding(gen_ccht_bio, basis='X_umap', color='celltype', title="Biopsy pSC UMAP", legend_loc="on data", legend_fontsize=7)
```
