...
```{python}
# placeholder image
study_schema = spacer_with_text(height=500, width=750)
```
...
top_biotypes = features_ev.sum().sort_values(ascending=False).head(6).index.tolist()
...
).properties(height=200, width=60)
...
```{python}
(study_schema & (featurecounts_chart | barplot_sc_sum).resolve_scale(color="independent"))
```
...
loadings_plt = alt.Chart(loadings_combined).mark_line(
  ...
  x=alt.X("Feature:N", sort=None).axis(labelAngle=-45),
...
plt_width, plt_height = (1000, 130) # Plot dimensions
...
base_chart = alt.Chart(stat_comb).encode(
  x=alt.X("comparison:N", sort=comp_order).axis(labels=True, labelAngle=-30, ticks=True, title=None),
...
# set up basechart for the Y axis (previously X) and X axis (previously Y)
base_chart = alt.Chart(stat_comb).encode(
  x=alt.X("comparison:N", sort=comp_order).axis(labels=True, labelAngle=-30, ticks=True, title=None, titleY=25),
...
sq.pl.spatial_scatter(
  ...
  color=cts,
  crop_coord=crop_coords,
  size=1.3, title="Glandular - Generated pUF-EV",
  figsize=(6, 3),
)
sq.pl.spatial_scatter(
  ...
  color=cts,
  crop_coord=crop_coords,
  size=1.3, title="Glandular - Generated Tissue",
  figsize=(6, 3),
)
```
...
def plot_lineage_umap(adata_bio, adata_ev, lineage, color_map_dict, figsize=(8, 3)):
  ...
  fig.suptitle(f"{lineage} Lineage", fontsize=14, y=1.02)
...
for lineage in unique_lineages_bio:
  plot_lineage_umap(gen_ccht_bio, gen_ccht_ev, lineage, color_map_dict)
```
...
```{python}
supp_1 = (
  (biotype_prop & qualimap_plot).resolve_scale(color="independent") |
