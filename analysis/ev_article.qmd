...
volcano_base = alt.Chart(
  ...
  color=alt.condition(alt.datum.pval < 0.05, alt.value("red"), alt.value("grey"))
)

## the text for the top two diff celltypes
volcano_annot = alt.Chart(
  diff_df.reset_index().sort_values("pval").head(5).assign(text_angle=lambda x: np.where(x["diff"] < 0, 330, 30))
...
## the dividing line
rule_tmp = pd.DataFrame({"threshold_p": [ .05 ]})
...
x_min, x_max = (dendro_coord[cols_xk].min(axis=None), dendro_coord[cols_xk].max(axis=None))
dendro_coord[cols_xk] = (dendro_coord[cols_xk] - x_min) / (x_max - x_min)
dendro_coord[cols_xk] *= (dendro_marks_x.shape[0] - 1)
dendro_coord[cols_yk] *= scaler/dendro_coord[cols_yk].max(axis=None) # scale the y heights
dendro_coord[cols_yk] += 1.18 # and shift up whole unit
...
# the U shape is composed of a shoulder plus two arms, made with separate lines...
shoulder = base.mark_rule().encode(
    x=alt.X("xk2:Q", title="").axis(grid=False).scale(domain=[0,len(dendro_marks_x)-1]),
    x2=alt.X2("xk3:Q"),
    y=alt.Y("yk2:Q", title="").axis(None))
arm1 = base.mark_rule().encode(
    x=alt.X("xk1:Q").axis(grid=False).scale(domain=[0,len(dendro_marks_x)-1]),
    y=alt.Y("yk1:Q").axis(None),
    y2=alt.Y2("yk2:Q"))
arm2 = base.mark_rule().encode(
    x=alt.X("xk3:Q").axis(grid=False).scale(domain=[0,len(dendro_marks_x)-1]),
    y=alt.Y("yk3:Q").axis(None),
    y2=alt.Y2("yk4:Q"))

# chart_den = (shoulder + arm1 + arm2).properties(width=.98*plt_width, height=50)
chart_den_x = shoulder + arm1 + arm2
```
...
x_min, x_max = (dendro_coord[cols_xk].min(axis=None), dendro_coord[cols_xk].max(axis=None))
dendro_coord[cols_xk] = (dendro_coord[cols_xk] - x_min) / (x_max - x_min)
dendro_coord[cols_xk] *= (dendro_marks_y.shape[0] - 1)
dendro_coord[cols_yk] *= scaler/dendro_coord[cols_yk].max(axis=None) # scale the y heights
dendro_coord[cols_yk] += 1.18 # and shift up whole unit
...
# the U shape is composed of a shoulder plus two arms, made with separate lines...
shoulder = base.mark_rule().encode(
    y=alt.Y("xk2:Q", title="").axis(grid=False).scale(domain=[0,len(dendro_marks_y)-1]),
    y2=alt.Y2("xk3:Q"),
    x=alt.X("yk2:Q", title="").axis(None).scale(reverse=True))
arm1 = base.mark_rule().encode(
    y=alt.Y("xk1:Q").axis(grid=False).scale(domain=[0,len(dendro_marks_y)-1]),
    x=alt.X("yk1:Q").axis(None).scale(reverse=True),
    x2=alt.X2("yk2:Q"))
arm2 = base.mark_rule().encode(
    y=alt.Y("xk3:Q").axis(grid=False).scale(domain=[0,len(dendro_marks_y)-1]),
    x=alt.X("yk3:Q").axis(None).scale(reverse=True),
    x2=alt.X2("yk4:Q"))

# chart_den = (shoulder + arm1 + arm2).properties(width=.98*plt_width, height=50)
chart_den_y = shoulder + arm1 + arm2
```
...
# add the color groupings 
x_group_colors = base_heatmap.mark_rect().encode(
  x=alt.X('x_vars:O', sort=dendro_marks_x, title=None), # Nominal for categorical x-axis
  color=alt.Color("cyclephase:N"),
  xOffset=alt.value(10), # set it to +10 of the plot height so that it would overlap
)

y_group_colors = base_heatmap.mark_rect().encode(
  y=alt.Y('y_vars:O', sort=dendro_marks_y, title=None), # Nominal for categorical x-axis
  color=alt.Color("cyclephase_y:N").legend(None),
  yOffset=alt.value(10), # set it to +10 of the plot height so that it would overlap
)
...
```{python}
dendro_corrplot = (final_chart + chart_den_x + chart_den_y).resolve_scale(x="shared", y="shared", color="independent").resolve_axis(y="independent").properties(width=200, height=500)
