...
# Prelude: Set up the environment
...
The HECA dataset is renormalised with the `omicsverse` v1.6.7 package's `preprocess` function with the following command `ov.pp.preprocess(sc_dat, mode="pearson|pearson", n_HVGs=3000)`. This normalises the dataset with the Pearson method (??? et al (check the omicsverse documentation)).
...
The spatial slides with spatial deconvolution
...
# gen_st_file = Path(anndata_folder).expanduser() / "c2l_st_sc.h5ad"
# vis_dat_sc = sc.read_h5ad(gen_st_file, backed="r").to_memory()
...
In this chapter we are projecting the single cell datasets generated from the bulk RNA-seq samples to the Spatial Transcriptomic Visium slides. For this, we are using the Tangram + cell2location method. The Tangram method is a deep learning-based approach that uses a neural network to learn the spatial distribution of cell types from the single-cell data and then applies this learned distribution to the spatial transcriptomic data. The cell2location method is a probabilistic model that uses the spatial transcriptomic data to infer the abundance of cell types in each spot on the slide. The combinatino of these two methods allows us to generate a high-resolution spatial map of the cell types in the tissue.

Given the nature of the data, we expect to see a high correlatino between the projected biopsy and the reference raw scRNA-seq data. The projected UF data should be more dispersed, as the UF samples are not directly comparable to the biopsy samples. Still, the UF samples should show a similar pattern of cell type distribution as the biopsy samples, per the assumption that the abundances of cell types in the UF samples indicate where the EV's are coming from in the tissue.

Therefore, in this chapter we are using two slides of the early-secretory tissue for this bulk2space method of projection. Then, we are summarising the results using various statistical methods for comparison between projection and the reference:

* Mean Average Difference (MAD) - the mean of the absolute differences between the projected bulk and the reference raw scRNA-seq data
* Centroid Distance (CD) - the distance between the centroids of the projected bulk and the reference raw scRNA-seq data
* Jaccard Index (JI) - the ratio of the intersection of the projected bulk and the reference raw scRNA-seq data to the union of the projected bulk and the reference raw scRNA-seq data. Here we have assigned the cell types to the spots based on the maximum abundance of the cell type in the spot. This is a common approach for calculating the Jaccard index in spatial transcriptomics, as it allows us to compare the cell type distributions between the projected bulk and the reference raw scRNA-seq data.

In addition to comparing between the projected bulk and the reference raw scRNA-seq data, we are also testing the clustering for every slide separately. This is done to see if the clustering is consistent across the slides. To test the significance of clusters, we are using Moran's I test. This test is used to assess the spatial autocorrelation of the cell types in the projected bulk and the reference raw scRNA-seq data. The Moran's I test measures the degree of clustering of the cell types.

But, first lets load the preprocessed datasets. The counts are normalised and the most variable genes (n=3000) are selected.
