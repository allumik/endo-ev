...
stats_df = pd.read_csv(Path(data_folder).expanduser() / "st_stats_table.csv")
```
...
Raw sequencing reads underwent preprocessing utilizing the nf-core/rnaseq pipeline (v3.14) [[Ewels et al](10.5281/zenodo.1400710)]. This process involved initial read trimming with Trim Galore! (v0.6.10) to remove adapter sequences and low-quality bases. Subsequently, the trimmed reads were aligned to the GRCh37.p13 reference genome, selected for its compatibility with previously published datasets, using the STAR aligner (v2.7.10a) [STAR](10.1093/bioinformatics/bts635). Following alignment, gene expression levels were quantified from the mapped reads employing the RSEM algorithm (v1.3.3) [RSEM](10.1186/1471-2105-12-323). Additionally, mapped reads were assigned to genomic features using [featureCounts](https://academic.oup.com/bioinformatics/article/30/7/923/232889?login=true) (v2.0.3). Both transcripts-per-million (TPM) and raw reads were used in downstream analysiss. The raw read counts were used for differential expression analysis, while the TPM values were employed for visualisation purposes.

Most of the preprocessing, statistical testing and visualisation was performed with Python (v3.9 for preprocessing and v3.11 for analysis and visualisation) programming language. The HECA dataset was normalized using the `preprocess` function from the `omicsverse` (v1.6.7) package with the command `ov.pp.preprocess(sc_dat, mode="pearson|pearson", n_HVGs=3000)`. This process applied Pearson residual normalization, selected the top 3000 highly variable genes (HVGs), and log-transformed the data. Pearson residuals were chosen due to their strong performance in a recent benchmarking study [[Ahlmann-Eltze & Huber, 2023](https://www.nature.com/articles/s41592-023-01814-1)]. No additional cell filtering was performed, as quality control was previously conducted by [Mareckova et al. (2023)](https://doi.org/10.1101/2023.11.03.564728). The resulting log-transformed data was used for downstream analysis, where scaling, dimensionality reduction with PCA and UMAP and calculating a neighborhood graph of observations using the [`scanpy` (v1.11.1) suite of analytical tools](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-017-1382-0). We combined the annotation for the celltypes from the main HECA dataset and the immune cell subtypes from the immune cell atlas. Due to the low number of representative cells in some rare subtypes, we merged them into broader categories. Specifically, 'uNK1', 'uNK1_cycling', 'uNK2', 'uNK3', 'ILC3', 'Peripheral_lymphocyte', 'Lymphatic', and 'Immune_Lymphoid' were grouped into 'Lymphatic'; 'eM1', 'eM2', 'cDC1', 'cDC2', 'pDC', 'Monocyte', and 'Immune_Myeloid' were grouped into 'Myeloid'; 'B_cell' and 'Plasma_B_cell' were grouped into 'B-cells'; and 'T_Reg', 'T_cell_CD8', 'T_cell_CD4', and 'T_cell_cycling' were grouped into 'T-cells'.
...
>Not used currently in the report, but left in for future reference.
...
Sample quality control (QC) procedures were performed to identify and exclude outlier samples prior to downstream analysis. Exclusion criteria were based on sample clustering characteristics and read count metrics. Based on these criteria, seven samples were excluded, and one sample was reassigned. Samples were removed if identified as distinct outliers or exhibited inconsistent clustering with other samples from their assigned experimental group following Uniform Manifold Approximation and Projection (UMAP) dimensionality reduction (Supplementary Figures). Furthermore, inter-sample correlation based on all detected genes was calculated; samples demonstrating low correlation with replicates within their group and clustering separately via hierarchical clustering were excluded (Supplementary Figures). Additionally, samples exhibiting low mRNA read content, defined as less than 5% of total reads mapping to miRNA features according to featureCounts, were removed, as this potentially indicated issues with input material quantity, particularly noted in vesicle samples. Three technical replicates were included for each sequencing batch associated with biopsy samples. These replicates demonstrated high inter-sample correlation (Spearman's ρ > X.XX [placeholder: insert value]), but two out of three were excluded from downstream analysis to reduce sample-specific bias. Finally, based on clustering, one paired UF sample was switched with its corresponding pair.


## Spatial transcriptomic deconvolution and projection

Visium slides were acquired from [Garcia-Alonso et al.](https://www.nature.com/articles/s41588-021-00972-2) and the ENA repository [E-MTAB-9260](https://www.ebi.ac.uk/biostudies/arrayexpress/studies/E-MTAB-9260). Preprocessing for the spatial analysis was performed using the Omicsverse `ov.pp.preprocess()` function with the following parameters: `mode="pearson|pearson"`, `n_HVGs=3000`, `log=True`, and `scale=True`. We selected the early-secretory samples (slide IDs 152807 and 152811) as the main focus of the analysis due to their relevance to endometrial receptivity. We project the single-cell datasets generated from the bulk RNA-seq samples of either EV, biopsy or scRNA-seq directly onto the Spatial Transcriptomic Visium slides using a combined [Tangram](https://www.nature.com/articles/s41592-021-01264-7) (v1.0.4) and [cell2location](https://doi.org/10.1038/s41587-021-01139-4) (v0.1.4) approach with default parameters. Tangram, a deep learning method, learns the spatial distribution of cell types from single-cell data and applies this to the spatial transcriptomic data. Cell2location, a probabilistic model, infers cell type abundance in each spatial spot. The combination of these methods allows us to generate a high-resolution spatial map of cell types within the tissue.

## Deconvolutional model parameters

>Create supplementary figures visualising the training loss and the model performance.

The [TAPE](https://www.nature.com/articles/s41467-022-34550-9) deconvolution model was configured using several key parameters for optimal performance in this study. We trained the model using the [HECAv2](https://doi.org/10.1101/2023.11.03.564728) endometrial cell atlas, as described before. To focus the model on the most informative features, the top 250 marker genes were selected (`top_marker_num=250`). For computational efficiency, the maximum number of single cells used for training was set to one-eighth of the total available single-cell profiles (`max_single_cells = round(len(sc_dat.obs.index) / 8)`). Although initial loss plot analysis suggested convergence around 500 epochs, the training was extended to 1000 epochs (`epochs=1000`) to potentially enhance the accuracy of cell type representation, as indicated by supplementary figures visualising training loss. The StandardScaler (`scaler="ss"`) was chosen for data scaling to better harmonise feature distributions. Furthermore, the model was run in "high-resolution" mode (`mode="high-resolution"`), which integrates a Variational Autoencoder (VAE) component for improved cell type inference. The `ratio_num` parameter, influencing synthetic bulk sample generation, was set to one.

Due to the generational nature of the model, the trained deconvolution model was used to generate pseudo-single-cell (pSC) data from the bulk samples. This process involved the generation of synthetic single-cell profiles from the bulk RNA-seq data, one pSC dataset per bulk data type. All the bulk and biopsy samples were used for training the model, and the pSC data was generated using the same parameters as the training. The pSC data was then preprocessed the same way as the original scRNA-seq atlas by normalisation, scaling, dimensionality reduction, calculating a neighborhood graph of observations and finally visalised (Fig 5).
...
## Spatial clustering and deconvolution statistics

Projection of bulk RNA-seq data onto spatial coordinates was performed using the bulk2space method on two histological slides of early-secretory tissue. The accuracy of the resulting spatial cell type mapping was assessed by comparison against reference single-cell RNA sequencing (scRNA-seq) data using multiple metrics. These included the Mean Absolute Difference (MAD), calculated as the mean of absolute differences in cell type abundance between projected and reference data; the Centroid Distance (CD), representing the distance between the centroids of the projected and reference cell type spatial distributions; and the Jaccard Index (JI). For JI computation, spots were assigned to the cell type exhibiting the maximum projected abundance, followed by calculation of the intersection over union with the reference cell type assignments. Additionally, spatial clustering was performed independently on each projected slide to evaluate consistency. The statistical significance and spatial autocorrelation of the resulting cell type clusters were assessed using Moran’s I test.
...
```{python}
sc.pl.embedding(gen_ccht_ev, basis='X_umap', color='lineage', frameon='small')
sc.pl.embedding(gen_ccht_bio, basis='X_umap', color='lineage', frameon='small')
```
...
boxplot = alt.Chart(filtered_df).mark_boxplot(extent='min-max').encode(
  x=alt.X('lineage:N', title='Lineage', axis=alt.Axis(labelAngle=90)),  # Rotate x-axis labels
  ...
  tooltip=['metric', 'lineage', 'comparison', 'value']  # Add tooltips for interactivity
)

# 2. Apply faceting
facet_chart = boxplot.facet(
  column=alt.Column('metric:N', title=None, header=alt.Header(labelOrient='top')),  # Facet by metric
).resolve_scale(
  x='independent',
  y='independent'
).properties(
  columns=col_wrap,  # Wrap facets into specified number of columns
  title=alt.TitleParams(
      'Comparison Metrics: EV vs. BIO relative to REF (across slides)',
      anchor='middle'  # Center the overall title
  )
)
...
final_chart = chart_layers.facet(
...
# Define the base chart for layering (data defined here is fine for layers)
base = alt.Chart().encode(
  x=alt.X('Moran I', axis=alt.Axis(title="Moran's I", grid=False)),
  y=alt.Y(
    'Moran I FDR',
    scale=alt.Scale(type='log', reverse=True), # Log scale, inverted
    axis=alt.Axis(title="Moran's I FDR (log scale)", values=tick_values) # Title and specific ticks
  ),
  color=alt.Color('celltype', legend=alt.Legend(title="Celltypes", orient='bottom-right', offset=10)), # Color by celltype, set legend title
  tooltip=['slide_id', 'celltype', 'lineage', 'comparison', 'Moran I', 'Moran I FDR'] # Tooltips on hover
)

# Create the scatter points layer
points = base.mark_point(filled=True, size=60, opacity=0.8)

# Create the line layer connecting points within each celltype per facet
lines = base.mark_line(opacity=0.2).encode(
  order='Moran I' # Connect points based on Moran I value
)

# Create the horizontal rule layer at y=0.05
# This layer doesn't need the main dataframe
rule = alt.Chart().mark_rule(color='lightgrey', strokeDash=[5,5], opacity=0.2).encode(
  y=alt.datum(0.05)
)

# Layer the chart elements: rule first (bottom), then lines, then points
chart_layers = rule + lines + points

# Apply faceting
# *** Explicitly pass the data to the facet method ***
final_chart = chart_layers.facet(
  data=moran_pivot,
  column=alt.Column('lineage:O', header=alt.Header(titleOrient="top", labelOrient="top"), title=None), # Facet by lineage
  columns=3 # Specify number of columns for wrapping
).resolve_scale(
  x='independent', # Don't share x-axis scale/limits
  color='independent' # Don't share color scale/limits
).properties(
  title=alt.TitleParams(
    text=f'{metric_to_plot}: EV vs REF & BIO vs REF',
    anchor='middle'
  )
...
`Comment: as these plots can't be visualised using a single framework (contains both matplotlib and Altair libraries), then these are the subplots that are generated separately and compiled later manually`
