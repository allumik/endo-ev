...
frac_clin_pred = pd.read_table(fractions_file, sep="\t", index_col=0)
## we select only the EF samples that had the same protocol
frac_clin_pred = frac_clin_pred.query("~samplename.str.startswith('EF')")
```
...
fig1 = peruvian_sands(
  fractions_df=peruvian_transform(
      fractions=frac_clin_pred, phenotype=clin_uf_pheno.query("n_transfer_embryo==1"), grouping_ids=["implantation"]
    ), 
...
fig2 = peruvian_sands(
  fractions_df=peruvian_transform(
      fractions=frac_clin_pred, phenotype=clin_uf_pheno.query("n_transfer_embryo==2"), grouping_ids=["implantation"]
    ), 
  x_axis_col="implantation",
  x_title="Implantation?",
...
biplot_fractions(
  frac_clin_pred, clin_uf_pheno, 
  legend_title="Implantation", 
  color_field="implantation",
  style_field="n_transfer_embryo"
)[0]

```

Project clinical samples to biopsy reference.
...
frac_long = convert_to_long(frac_clin_pred, clin_uf_pheno).drop(columns=["n_transfer_embryo"])
