...
pearson_mat, jaccard_mat, lees_mat, lees_pval_mat = calculate_correlations(
  vis_dat, 
...
# plot_correlation_heatmap(lees_mat, f"Slide {slide_id}", "Bivariate lees's I")

# 3. Bivariate lees's I (WITH Masking)
# Pass the p-value matrix computed in the previous step
plot_correlation_heatmap(
    lees_mat, 
    f"Slide {slide_id}", 
    "Bivariate Lee's L (Significant Only)", 
    pval_df=lees_pval_mat, 
    sig_threshold=0.05
)
...
# Run Aggregation
agg_pearson, agg_jaccard, agg_lees, agg_pval = aggregate_correlations(
  slide_ids,
  adata_collection, # Your main dictionary of slides
  all_genes,        # Your gene dictionary
  OBSM_KEY
)
results_prolif = (agg_lees, agg_pval)

# Plot the Aggregated Results
plot_correlation_heatmap(
    agg_lees, 
    "Proliferative Phase (Combined)", 
    "Avg Bivariate Lee's L", 
    pval_df=agg_pval, 
    sig_threshold=0.05
)
```
...
results_early = (agg_lees, agg_pval)
...
Here we take the leess I results and summarize them in a more digestable visual format.
...
# print(
  # np.max([ np.max(results_prolif[0]), np.max(results_early[0]), np.max(results_mid[0]) ])
# )

# print(
  # np.min([ np.min(results_prolif[0]), np.min(results_early[0]), np.min(results_mid[0]) ])
# )
```
...
But first, we look at univariate Morans' I to check for specificity of the expression of cell types. This would help us filter out cell types that are more spatially clustered, i.e., more morphologically specific. NOTE: This plot would mirror the analysis presented already in ev2space_stats.qmd, so just look at the moran's I values presented there.

```{python}

## NB: just look at ev2space_stats
## read in the stats table from the ev2space_stats.qmd
# uni_results = pd.read_csv(Path(data_folder).expanduser() / "st_stats_table.csv", index_col=0)
# uni_results.assign(slide_id=lambda x:x.slide_id.astype(str)).query("metric == 'Moran I' & comparison == 'ref' & slide_id in @slide_ids")

# TODO: convert the df above to dictionary so it works with the function
...
for phase in phases.keys():
  _, _, agg_lees, agg_pval = aggregate_correlations(phases[phase], adata_collection, unified_genes, OBSM_KEY)
  phase_stats = (agg_lees, agg_pval)
...
averaged_results = (
  pd.concat([cycle_all_ev["Proliferative"][0], cycle_all_ev["Early-Secretory"][0], cycle_all_ev["Mid-Secretory"][0]])
  .query("index == 'eStromal'") # uSMCs eStromal
