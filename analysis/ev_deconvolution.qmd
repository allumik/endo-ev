...
funk.peruvian_sands(
  fractions_df=funk.peruvian_transform(fractions=frac_pred, phenotype=comb_uf_pheno, grouping_ids=["cyclephase"]), 
  legend_col_n=2
).show()
...
```{python}
funk.biplot_fractions(frac_pred, comb_uf_pheno, color_field="cyclephase", style_field="group")[0]
```
...
funk.dendro_barplot(frac_pred_gen, comb_uf_pheno, legend_class_ncol=2)
funk.peruvian_sands(
  fractions_df=funk.peruvian_transform(
    fractions=frac_pred.filter(focus_celltypes, axis=1), phenotype=comb_uf_pheno, grouping_ids=["cyclephase"]
    ),
  legend_col_n=1
).show()
funk.biplot_fractions(frac_pred_gen, comb_uf_pheno, color_field="cyclephase", style_field="group")[0]
"""
...
frac_long = funk.convert_to_long(frac_pred.loc[comb_uf_pheno.query("group == 'biopsy'").index], comb_uf_pheno)

# perform the tukey hsd analysis
tukey_res = funk.tukey_table(frac_long)

# perform the ANOVA per cell type
anova_res = funk.anova_table(frac_long)

# perform rank correlation with dummy vars
cor_res = funk.cor_table(frac_long)
...
funk.peruvian_sands(
  fractions_df=funk.peruvian_transform(fractions=frac_tmp, phenotype=comb_uf_pheno, grouping_ids=["cyclephase"]), 
  legend_col_n=2
).show()
...
```{python}
funk.biplot_fractions(frac_tmp, comb_uf_pheno, color_field="cyclephase", style_field="group")[0]
```
...
funk.biplot_fractions(frac_pred_gen, comb_uf_pheno, color_field="cyclephase", style_field="dataset")[0]
"""
...
frac_long = funk.convert_to_long(frac_tmp, comb_uf_pheno)
...
```{python}
funk.biplot_fractions(frac_tmp, comb_uf_pheno, color_field="cyclephase", style_field="dataset")[0]
```
...
funk.peruvian_sands(
  fractions_df=funk.peruvian_transform(fractions=frac_pred, phenotype=ccht_uf_pheno, grouping_ids=["cyclephase"]), 
  legend_col_n=2
).show()
...
```{python}
funk.biplot_fractions(frac_pred, ccht_uf_pheno, color_field="cyclephase", style_field="group")[0]
```
...
funk.dendro_barplot(frac_pred_gen, ccht_uf_pheno, legend_class_ncol=2)
funk.peruvian_sands(
  fractions_df=funk.peruvian_transform(
...
funk.biplot_fractions(frac_pred_gen, ccht_uf_pheno, color_field="cyclephase", style_field="group")[0]
"""
...
frac_long = funk.convert_to_long(frac_pred.loc[ccht_uf_pheno.query("group == 'biopsy'").index], ccht_uf_pheno)
...
funk.peruvian_sands(
  fractions_df=funk.peruvian_transform(fractions=frac_tmp, phenotype=ccht_uf_pheno, grouping_ids=["cyclephase"]), 
  legend_col_n=2
).show()
...
```{python}
funk.biplot_fractions(frac_tmp, ccht_uf_pheno, color_field="cyclephase")[0]
```
...
funk.biplot_fractions(frac_pred_gen, ccht_uf_pheno, color_field="cyclephase")[0]
"""
...
frac_long = funk.convert_to_long(frac_tmp, ccht_uf_pheno)
...
frac_long = funk.convert_to_long(frac_pred, scratch_pheno)
...
frac_long = funk.convert_to_long(frac_pred, clin_uf_pheno).drop(columns=["n_transfer_embryo"])
...
frac_ccht = funk.convert_to_long(frac_ccht_mat, ccht_uf_pheno.query("group == 'biopsy'"))
  ...
  for celltype in frac_ccht.celltype.unique():
    ...
    for samplename in frac_clin.index:
      list_to_df.append([
        ...
        celltype,
        samplename,
        funk.mahalanobis_distances(frac_clin.loc[samplename, celltype], ref_vals)
      ])
...
for cyclephase in frac_ccht.cyclephase.unique():
  mah_means.update({
    ...
    .apply(funk.geometric_mean, axis=1)
    # .mean(axis=1)
  })
...
frac_clin = frac_pred # reuse already processed file
frac_ccht_mat = pd.read_table(fractions_folder / "ccht_fracs.tsv", sep="\t", index_col=0)
frac_ccht = funk.convert_to_long(frac_ccht_mat, ccht_uf_pheno.query("group == 'UF'"))
