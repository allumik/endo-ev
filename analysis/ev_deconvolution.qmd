...
comb_uf_raw = pd.read_feather(Path(data_folder).expanduser() / "combined" / "comb_uf_batch.feather").set_index("gene_id").iloc[:-1]
...
comb_uf_raw = comb_uf_raw.drop(columns=terminator[0])
...
peruvian_sands(
  fractions_df=peruvian_transform(fractions=frac_pred, phenotype=comb_uf_pheno, grouping_ids=["cyclephase"]), 
  legend_col_n=2
).show()
...
```{python}
biplot_fractions(frac_pred, comb_uf_pheno, color_field="cyclephase", style_field="group")[0]
```
...
biplot_fractions(frac_pred_gen, comb_uf_pheno, color_field="cyclephase", style_field="group")[0]
"""
...
```{python}
# perform the tukey hsd analysis
frac_long = convert_to_long(frac_pred.loc[comb_uf_pheno.index], comb_uf_pheno)

ttest_res = ttest_table(frac_long, testing_col="group", grouping_col="celltype")

ttest_res.to_excel(fractions_folder / "comb_comp.xlsx", sheet_name="bio_uf_comp")
```
...
```{python}
biplot_fractions(frac_tmp, comb_uf_pheno, color_field="cyclephase", style_field="dataset")[0]
```
